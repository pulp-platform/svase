...
void Design::insertInstance(const InstanceSymbol *const sym, const size_t collisions) {
    ...
    if (!emplRet.second) {
        ...
        uniqModByName.emplace(std::piecewise_construct, std::make_tuple(containedMod->getUniqueName()),
                              std::make_tuple(genericName, id));
        uniqModByParamStr[genericName].emplace(
                std::piecewise_construct, std::make_tuple(genParamString(sym)), std::make_tuple(id));
    }
}
...
void Design::walkModuleInstances(const Symbol *sym) {
    switch (sym->kind) {
        ...
        case SymbolKind::Instance: {
            ...
            if (instSymb->isModule()) insertInstance(instSymb);
...
const std::unordered_map <size_t, DesignUniqueModule> *Design::getGenericModule(std::string_view genericName) const {
    if (auto find = uniqMods.find(genericName); find == uniqMods.end()) return nullptr;
    else return &find->second;
}

// Get a uniquified module by its generic name and ID if it exists or null.
DesignUniqueModule *Design::getUniqueModule(std::string_view genericName, size_t id) const {
    auto module = getGenericModule(genericName);
    if (!module) return nullptr;
    auto find = module->find(id);
    if (find == module->end()) return nullptr;
    else return const_cast<DesignUniqueModule *>(&find->second);
}

DesignUniqueModule *Design::getUniqueModule(std::string uniqueName) const {
    if (auto find = uniqModByName.find(uniqueName); find == uniqModByName.end()) return nullptr;
    else return getUniqueModule(find->second.first, find->second.second);
}
...
DesignUniqueModule *Design::getUniqueModule(std::string_view genericName, std::string paramString) const {
    if (auto find = uniqModByParamStr.find(genericName); find == uniqModByParamStr.end()) return nullptr;
    else if (auto inFind = find->second.find(paramString); inFind == find->second.end()) return nullptr;
    else return getUniqueModule(genericName, inFind->second);
}

DesignUniqueModule *Design::getUniqueModule(const InstanceSymbol& instSym) const {
    return getUniqueModule(instSym.getDefinition().name, genParamString(&instSym));
}
