...
cxxopts::Options genCmdOpts () {
    cxxopts::Options ret("svase", "SVase: a source-to-source SystemVerilog elaborator");
    ...
    ret.add_options()
            ...
            ("top",         "Top module of design to elaborate",  cxxopts::value<std::string>())
            ("out",         "The output file (- for stdout) or library", cxxopts::value<std::string>())
            ("files",       "The source files to process", cxxopts::value<std::vector<std::string>>())
            // Optional arguments
            ("l,lib",           "Output library of individual modules", cxxopts::value<bool>()->implicit_value("false")) // TODO
            ("z,compress",      "Compress output file or library using Gzip", cxxopts::value<bool>()->implicit_value("false")) // TODO
            ("s,slang-argfile", "Argument file overriding Slang default options", cxxopts::value<std::string>())
            ("S,slang-args",    "Argument string overriding Slang default options", cxxopts::value<std::string>())
            ("r,timetrace",     "Time each stage and write chrome event trace to JSON", cxxopts::value<std::string>()) // TODO
            ("V,verbosity",     "Verbosity of stderr diagnostics: 3(errors), 2(warnings), 1(notes)", cxxopts::value<uint>()->default_value("2"))
...
int driverMain(int argc, char** argv) {
    ...
    try {cmdOptsRes = cmdOpts.parse(argc, argv);}
    ...
    auto slangCmd = fmt::format("{} {} {} --top {} {}", argv[0], slangArgs, builtinFlags,
   ...
   // } catch (const std::exception e) {diag.log(DiagSev::Fatal, e.what()); ok = false;}
    ...
    // TODO: Postprocess syntax tree into writable buffers, one per root unit member
    ...
    try {
        ...
        postBuffers.emplace_back(cmdOptsRes["top"].as<std::string>(), synTree->root().toString());
    } catch (const std::exception e) {diag.log(DiagSev::Fatal, e.what()); ok = false;}
    if (!ok) return 7;
    ...
    //try {
        ...
        // TODO: library handling and such; guard that out exists, is legal beforehand!
...
int main(int argc, char** argv) {
    return svase::driverMain(argc, argv);
}
