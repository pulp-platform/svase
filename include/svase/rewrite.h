...
/// with their elaborated values. This requires uniquified modules as generated
/// by `UniqueModuleWriter`.
class DefaultAssignmentRewriter
    : public DesignRewriter<DefaultAssignmentRewriter> {
private:
public:
  using DesignRewriter::DesignRewriter;

  void handle(const StructuredAssignmentPatternSyntax &pd);
  void handle(const AssignmentPatternItemSyntax &pd);
  void handle(const AssignmentPatternExpressionSyntax &pd);
};
