...
namespace svase {
    ...
    private:
        s
        TypedBumpAllocator<std::string> &postBuffers;
        Diag &diag;

    public:
        Postprocessor(std::vector<SourceBuffer>& buffers, TypedBumpAllocator<std::string> &postBuffers, Diag &diag) :
        ...
        void filterPragmaTranslate(std::string &strBuf, BufferID& id) {
            std::regex re(R"(//\s*pragma\s+translate[_ ]off(.|\n)+?//\s*pragma\s+translate[_ ]on)");
            std::smatch match;
            ...
            while (std::regex_search(start, strBuf.cend(), match, re)) {
                ...
                diag.log(DiagSev::Note, "Filtering out `pragma_translate off` region before parsing",
                         SourceLocation(id, begin - strBuf.cbegin().base()), false, false);
                std::fill(begin, end, ' ');
